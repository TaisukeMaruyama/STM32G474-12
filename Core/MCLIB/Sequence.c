...
void slctCntlFromDrvMode(uint8_t drvMode, struct SensorData sensData, struct VectorControlData *vectorControlData, float* Duty, int8_t* outputMode){
	...
	switch(drvMode){
		...
		case DRVMODE_OPENLOOP:
			 VamRef = sSensData.Vdc * SQRT3DIV2_DIV2 * ( 0.1f + 0.25f * sElectAngVeloRefRateLimit*0.0005f);//sElectAngVeloRefRateLimit * 0.001 );
			//  VamRef = sSensData.Vdc * SQRT3DIV2_DIV2 * 0.2f * gButton1;; //sElectAngVeloRefRateLimit * 0.001 );
